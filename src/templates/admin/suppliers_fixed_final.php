...
function fetchSuppliers($con)
{
    ...
    // Debug: First check what's in the tables
    error_log("=== DEBUG: Checking suppliers ===");
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            // Debug logging
            error_log("User: " . $row['user_name'] . " - BR ID: " . ($row['br_id'] ?? 'NULL') . " - Certificate: " . ($row['b_certificate'] ?? 'NULL'));
            $suppliers[] = $row;
        }
    } else {
        error_log("No suppliers found or query failed: " . mysqli_error($con));
    }
...
<div class="bg-gray-800 min-h-screen p-6">
    <div class="bg-gray-900 p-8 shadow-xl rounded-xl">
        ...
        <div class="mb-4 p-4 bg-gray-700 rounded-lg">
            <p class="text-yellow-400 text-sm">Debug: Found <?= count($suppliers) ?> suppliers</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-700">
                    <tr>
                        <th class="py-3.5 px-4 text-left text-sm font-semibold text-gray-300">User ID</th>
                ...
                <tbody class="divide-y divide-gray-800 bg-gray-800">
                        ...
                        <?php foreach ($suppliers as $supplier): ?>
                            <tr>
                                <td class="px-4 py-4 text-sm font-medium text-gray-200">
                                    <?= htmlspecialchars($supplier['user_id']) ?>
                                </td>
                                <td class="px-4 py-4 text-sm font-medium text-gray-200">
                                ...
                                <td class="px-4 py-4 text-sm text-gray-400">
                                    ...
                                    $hasBR = !empty($supplier['b_certificate']) && !is_null($supplier['br_id']);
                                    $brPath = $supplier['b_certificate'];
                                    ?>

                                    <?php if ($hasBR): ?>
                                        <div class="flex items-center space-x-2">
                                            <button onclick="openBRModal('<?= htmlspecialchars($brPath) ?>', '<?= htmlspecialchars($supplier['user_name']) ?>')"
                                        ...
                                        <div class="text-xs text-gray-500 mt-1">
                                            Path: <?= htmlspecialchars($brPath) ?>
                                        </div>
                                    <?php else: ?>
                                        <span class="text-gray-500 text-xs">Not uploaded</span>
                                        <!-- Debug info -->
                                        <div class="text-xs text-red-400 mt-1">
                                            BR ID: <?= $supplier['br_id'] ?? 'NULL' ?> |
                                            Cert: <?= $supplier['b_certificate'] ?? 'NULL' ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                            </tr>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="px-4 py-4 text-center text-sm text-gray-400">No suppliers found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
